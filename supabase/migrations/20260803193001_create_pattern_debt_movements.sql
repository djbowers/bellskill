-- Pattern-debt ledger Phase 1: per-movement aggregation RPC.
--
-- pattern_debt_movements returns raw per-movement window + baseline aggregates
-- plus the catalog row's pattern_credits. ALL attribution (credit fan-out,
-- get-up name regex, unlinked-movement policy) lives in the shared TS scorer
-- (src/utils/patternDebt.ts) — see docs/pattern-debt-scoring-model.md.
--
-- The legacy 7-pattern pattern_debt_window stays live until the PWA and both
-- recommender edge functions migrate (PWA service-worker client skew makes
-- in-place replacement unsafe); a follow-up migration drops it.
--
-- Volume rules mirror the client accumulator (and the legacy function):
-- complex sets read the session's shared weight; both weights count; reps/sets/
-- volume scale by completed rounds with the left/right mirroring rule; timed
-- rungs (seconds in rep_scheme) contribute recency but no reps/volume.
...
-- The RPC's per-user, per-window scan needs this to avoid a full table scan.
CREATE INDEX IF NOT EXISTS movement_logs_user_id_created_at_idx
  ON public.movement_logs (user_id, created_at DESC);
