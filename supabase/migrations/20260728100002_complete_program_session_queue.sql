-- complete_program_session: promote the queue when a program finishes.
--
-- When the enrollment's last session is satisfied and the user has queued
-- programs (see 20260728100000), the lowest queue_position row is promoted
-- into the slot this enrollment frees: the finisher flips to 'completed' and
-- the queued row becomes 'active' in the same transaction, its baked weights
-- untouched. The queued row is locked FOR UPDATE so two concurrent
-- completions cannot promote the same program twice.
--
-- Precedence: the queue beats auto_repeat. A queue entry is an explicit,
-- later, finite intent ("do this next"); auto-repeat is a standing loop that
-- would otherwise make the queue unreachable. With an empty queue, auto-repeat
-- loops exactly as before. Returns TRUE on promotion -- the program IS
-- complete; the client shows completion and the promoted program simply
-- appears in the freed slot.
--
-- Everything else is carried over verbatim from
-- 20260727000002_complete_program_session_auto_repeat.sql.
...
BEGIN
  ...
  IF v_all_done THEN
    ...
    IF v_next_id IS NOT NULL THEN
      -- Complete the finisher first so its slot is free for the promotion
